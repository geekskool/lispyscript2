...
module.exports = function(src) {
    ...
    var spaceRegEx = /^[ \t\r\n\b\f]+/,
        ...
        closeParanthesesRegEx = /^\)/,
        openSquareBracketRegEx = /^\[/,
        closeSquareBracketRegEx = /^\]/,
        rightCurlyBraceRegEx = /^\{/,
        leftCurlyBraceRegEx = /^\}/,
        operatorRegEx = /^(\*|\+|\-|\/|\%|\=|\<|\>)/,//fix-support conditional operators
        unaryOperatorRegEx = /^(\!)/,
    ...
    var spaceParser = consumer(spaceRegEx, (val) => {
        ...
        openParanthesesParser = consumer(openParanthesesRegEx, (val) => {
            return true
        }),
        closeParanthesesParser = consumer(closeParanthesesRegEx, (val) => {
            return true
        }),
        rightSquareBracketParser = consumer(openSquareBracketRegEx, (val) => {
            return true
        }),
        leftSquareBracketParser = consumer(closeSquareBracketRegEx, (val) => {
            return true
        }),
    ...
    function expressionStatementParser() {
        spaceParser()
        if (closeParanthesesParser())  return null
        return combinator({type: "ExpressionStatement"}, ["expression", expressionParser])
    }

    function macroCallParser() {
        var macro, macroId,val, temp = src
        if (!openParanthesesParser()) return null
        if ((macroId = macroIdentifierParser()) 
            && (macro = macros[macroId.name])) {
                
                var macroArgs = getArguments() || []
                var macroBody = macro.body

                if(!macroArgs.length) return null
                ...
                // fix to extract object from redundant array
    ...
    function unaryExpressionParser() {
        return combinator({type: "UnaryExpression"}, ["operator", unaryOperatorParser], ["argument", argStatementParser], ["prefix", true], ["", closeParanthesesParser])
    }
    function binaryExpressionParser() {
        return combinator({type: "BinaryExpression"}, ["operator", operatorParser], ["left", argStatementParser], ["right", argStatementParser], ["", closeParanthesesParser])
    }
