    ...
    function macroCallParser() {
        ...
        if (macroId) {
            if (macro = macros[macroId.name]) {
    			...
    			//fix - in case of zero args, return null if macroBody contains argument receiver
                if(!macroArgs.length) {
	               	//check the macro body for ~cond or ~...rest
                	let paramCheckRegEx = /\~[\.a-zA-Z]+/
                	if(paramCheckRegEx.test(macroBody))
                		return null
                }
